...
def process_subsection(individual_subsection):
  ...
  individual_subsection = individual_subsection.drop(list(individual_subsection.columns)[0],axis=1)
...
def process_trades_section(trades):
  ...
  trades[['Quantity', 'T. Price', 'Proceeds', 'Comm/Fee',
        'Basis', 'Realized P/L']] = pd.to_numeric(trades[['Quantity', 'T. Price', 'Proceeds', 'Comm/Fee',
        'Basis', 'Realized P/L']].stack()).unstack()
  return trades
...
def process_existing_entries(trading_journal_entries):
  ...
  existing_entries['Date/Time'] = pd.to_datetime(existing_entries['Date/Time'],errors='coerce')
  ...
  existing_entries = existing_entries[existing_entries['DataDiscriminator']=='Order']
  return existing_entries

def concat_new_entries(existing_entries,new_trades_entries):
  new_trades_entries = new_trades_entries.drop(['Header','Exchange'],axis=1)
  updated_entries = pd.concat([existing_entries,new_trades_entries])
...
def update_google_sheets(trading_journal_entries,updated_entries):
  ...
  trading_journal_entries.update([updated_entries.columns.values.tolist()] + updated_entries.values.tolist())
